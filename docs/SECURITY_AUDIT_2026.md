...
### RUK-SEC-009 — No security headers, CSP, or HSTS · **HIGH** · **Status: FIXED on the API (2026-09-04); CSP partially shipped on both static sites (2026-09-04) via meta tag; X-Frame-Options/HSTS/frame-ancestors on the static sites remain a Chunk 13 (CloudFront console) item**
...
**What shipped (2026-09-04):** `helmet` on the API via `backend/config/securityHeaders.js`. Every response now carries `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, `Cross-Origin-Resource-Policy: cross-origin` (so `app.riseup.kids` can still call `api.riseup.kids`), `Strict-Transport-Security` (max-age from `HSTS_MAX_AGE`, default 1 day — conservative and self-healing; `HSTS_MAX_AGE=0` disables it with no deploy), and helmet's other defaults; `X-Powered-By` is removed. **CSP is deliberately off** on the API — the API returns JSON, and the HTML pages (admin + sales) are static on CloudFront; their CSP belongs there. **Frame protection is deliberately off** on the API — a JSON API can't be clickjacked and the legacy content-player still frames HTML from this origin; the admin-SPA frame policy is a CloudFront item (Chunk 13). Verified live (`curl -I` against a running production-mode server) and by `backend/tests/securityHeaders.test.js` + `backend/tests/securityHeaders.e2e.test.js` (the latter drives the real `server.js` app).

**What shipped (2026-09-04, static-site half):** A `<meta http-equiv="Content-Security-Policy">` tag was added to `frontend/index.html` (admin/parent web app) and `riseupkids-sale/web/index.html` (sales site), plus the four standalone privacy/terms legal pages (very tight — `default-src 'none'; style-src 'unsafe-inline'` — since those are static docs with zero JavaScript). Both apps were built and manually verified in a real browser: zero CSP console violations across all reachable pages (admin login, sales home/videos/privacy/terms). Regression tests were added (`frontend/src/test/csp.test.js`, `riseupkids-sale/web/src/tests/cspMetaTag.test.js`) asserting the tag stays present with its key directives. **This is a partial fix** — a meta tag cannot set `X-Frame-Options`, `frame-ancestors`, `Strict-Transport-Security`, or `X-Content-Type-Options`; those require the CloudFront Response Headers Policy, documented and ready to paste into the AWS console at `docs/CLOUDFRONT_RESPONSE_HEADERS_POLICY.md`, still pending Chunk 13 (needs AWS console access).

### RUK-SEC-010 — Sensitive data in application logs at scale · **HIGH** · **Status: FIXED (2026-09-09)**
...
- **`backend/config/logger.js`** — a `pino` logger with a recursive scrubber that masks sensitive keys at any depth (passwords, all `*token*`/`*secret*` keys, JWTs, `Authorization`/`Cookie`, `client_secret`, API keys, connection strings with inline credentials, OTP/reset codes, card number/CVV, CPF/tax id) and child/user PII (display name, full name, DOB, phone, address, photo URLs; email partially masked). A `logMethod` hook scrubs the message string too, so a token interpolated into a template literal is caught. Errors serialize to `{ type, message, code }` with the stack only outside production, message and stack scrubbed. Dev output is human-readable (`pino-pretty`), production is JSON, tests are silent.
- **`server.js`** — `morgan` replaced with `pino-http`: request logs carry `req.id`, log the URL **without its query string**, drop `Authorization`/`Cookie`, and skip the health check. `customLogLevel` maps 5xx→error, 4xx→warn. The startup mail-config line no longer prints host/port — only `smtpConfigured: true|false`.
- **`middleware/errorHandler.js`** — logs via the scrubbing logger (per-request child when available), keyed by request id.
- **The sweep** — all **437 `console.*` calls across 58 files** in `controllers/`, `services/`, `jobs/`, `middleware/` now go through the logger. The four Stripe `JSON.stringify(...full object...)` debug dumps were replaced with one-line `logger.debug` of just the ids; the `[Book Completion]` request-tracing block and the YouTube-Live / SCORM / Meet diagnostic dumps were downgraded from `info` to `debug`.
- **CI gate** — `security.yml` fails the build if any `console.*` reappears in backend request-path code.
- **`docs/LOG_RETENTION_AND_ACCESS.md`** — draft retention (60-day hot) + access (SSH-only, named readers) + "no PII in alerts" policy; the numbers/names need client confirmation and the rotation transport is a Chunk 13 item.
- **Tests** — `logger.redaction.test.js` (12) unit-tests the scrubber; `logging.noSecrets.test.js` (7) drives representative payloads (user doc, auth error, JWT in a header and a message, webhook body, axios provider error, child profile, Mongo URI in an error) and asserts nothing sensitive reaches the captured output.
- **Not done here:** shipping logs to a central access-controlled store (Chunk 13); deleting the retired SCORM module (still routed — its 82 log calls were swept, not removed).
