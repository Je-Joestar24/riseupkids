...
> * *2026-09-10* — **iOS: Kids Wall excluded from the App Store build** (client-requested, App Store submission task, not a numbered chunk). Nav tab removed on iOS, the wall routes redirect to Home, the "coming soon" placeholder screen and its preview route deleted, and the iOS photo-library permission dropped (`expo-image-picker` `photosPermission: false`). Android/Web unchanged. 5 new tests; full app suite green (273/273). See `docs/IOS_KIDS_WALL_EXCLUSION.md`.
> * *2026-09-11* — **Chunk 7 (repo side) shipped.** Kids Wall consent is now opt-in end to end: `ChildProfile.kidsWallEnabled` defaults `false`; `hasKidsWallConsent()` requires the flag **and** a real timestamp; `createChild` no longer auto-grants; the parent-dashboard consent dialog (previously built but never wired in) now gates turning it on, recording the request IP. The cross-family feed (`GET /api/kids-wall/all`) is hard-scoped to approved posts from currently-consented children only, ignoring any caller-supplied filter. A migration resets children auto-consented by the old bug (consent timestamp within 60s of creation) while leaving genuine later consent untouched — **needs to run against production**. Account-deletion gaps closed: `DevicePushToken` + `NotificationReceipt` purged, plus an orphaned-`Media` sweep. `docs/CHILD_DATA_PRIVACY.md` + `docs/COPPA_LGPD_GAP_CHECKLIST.md` delivered. 22 new backend tests (2 real end-to-end against an in-memory MongoDB) + 9 new frontend tests; full backend suite 810/821 (only pre-existing unrelated failures), full frontend suite 68/68. Closes RUK-SEC-006 and RUK-SEC-026. **Needs the client:** run the migration in production; two open policy questions in the gap checklist.
>
...
> **Already done (fix-first, 2026-09-01):** the "JWT secret standard" item — `backend/config/jwtSecret.js` now rejects a missing/known-weak `JWT_SECRET` at startup (any env) and a sub-32-char one in production/staging; `.env.example` no longer ships a usable-looking default.
>
> **Done — repo side (2026-09-07).** Closes RUK-SEC-034.
> * Full-history secret scan (`detect-secrets` + pattern sweep) of the main repo and the `riseupkids-sale` submodule — **no true secret in history or working tree**. Only client-safe values were tracked: `app/google-services.json` (Android/Firebase config, restricted key) and `riseupkids-sale/web/.env` (Stripe *publishable* key + PayPal client IDs). Both untracked, `.example` added, `.gitignore`d. No rotation required; the one client action is verifying the Google API key's restrictions in the console.
> * Root `.gitignore` added (none existed) — `.env`, `*serviceAccount*.json`, `*.pem/.key/.p8/.p12`, `google-services.json`, build output, `.claude/`.
> * `backend/.env.example` + `app/.env.example` brought current — the AWS, PayPal, Google Vision, YouTube/Google OAuth, Expo-push, notification-scheduler and program-materials variables were all undocumented; real Flodesk segment IDs replaced with placeholders.
...
- [x] Child-data map + retention schedule documented. — `docs/CHILD_DATA_PRIVACY.md`, done 2026-09-11.
- [x] Deletion pipeline provably purges all child data + media (E2E passes). — `DevicePushToken`/`NotificationReceipt` + orphaned-`Media` sweep added; 3 tests in `tests/accountDeletion.service.test.js`. (The staging E2E walkthrough is deferred — staging is offline, same as noted in Chunk 6.)
- [x] Parental consent is opt-in and logged (timestamp + parent + IP). — done; see "still open" below for the *verifiable*-consent-strength question.
- [x] Third-party sharing (esp. Vision) documented and minimized. — already minimal (label detection only); documented in `docs/CHILD_DATA_PRIVACY.md`. No code change was needed.
- [x] COPPA/LGPD gap checklist delivered with owners + dates. — `docs/COPPA_LGPD_GAP_CHECKLIST.md` (owner/date columns left for the client to fill in — they're policy decisions).
- [ ] Privacy-policy change requests handed to client. — none needed; the current privacy policy's Kids Wall language already matches the fixed behavior.

**Status:** repo-side done. Closes RUK-SEC-006 and RUK-SEC-026. **Needs the client:** run `npm run migrate:kids-wall-consent-default` against production; decide the two open items in the gap checklist (consent-strength bar, and whether the cross-family feed design itself should change).
