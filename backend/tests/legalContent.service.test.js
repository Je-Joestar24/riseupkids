...
describe('legalContent.service', () => {
  ...
  it('returns production terms text (not placeholder)', () => {
    ...
    expect(result.version).toBe('2026-08-03');
    expect(result.content).toContain('Android app and Web only');
    expect(result.content).toContain('faces and body parts are not allowed');
    expect(result.content).toContain('schools, admins, and teachers');
  ...
  it('returns privacy URLs for clients', () => {
    ...
    expect(urls.version).toBe('2026-08-03');
  });
});
