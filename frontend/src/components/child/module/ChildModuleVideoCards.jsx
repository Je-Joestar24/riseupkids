...
/**
 * ChildModuleVideoCards Component
 * 
 * Individual video card component for Videos section
 * Displays cover image, completion status, star badge, time, and progress circles
 */
const ChildModuleVideoCards = ({
    video,
    isCompleted = false,
    ...
    // Videos use thumbnail or coverImage
    const coverImageUrl = video?.thumbnail || video?.coverImage 
        ? getCoverImageUrl(video.thumbnail || video.coverImage) 
        : null;

    // Get estimated time (videos have duration in seconds, format as "X min")
    const durationSeconds = video?.duration || 0;
    const durationMinutes = Math.ceil(durationSeconds / 60);
    const estimatedTime = durationMinutes > 0 ? `${durationMinutes} min` : '0 min';

    // Get star points (videos have starsAwarded field)
    const starPoints = video?.starsAwarded || 0;
...
export default ChildModuleVideoCards;
